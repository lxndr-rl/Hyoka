...
const phoneWidth = Platform.OS == 'web' ? Dimensions.get('window').width / 3 : Dimensions.get('window').width;

const Card = (props) => {
    const colors = [['#2b2b2b', '#2b2b2b']];
    const materia = Platform.OS == 'web' ? props.materia.length >= 30 ? `${props.materia.substring(0, 30).trim()}...` : props.materia : props.materia.length >= 20 ? `${props.materia.substring(0, 20).trim()}...` : props.materia;
    return (
        <LinearGradient
